  ...
  if(currentTab === 'works') {
    tbody.innerHTML = allItems.map(item => {
      ...
      return `<tr>
        ...
        <td class="text-muted"><i class="fa-regular fa-calendar"></i> ${item.completion_date ? new Date(item.completion_date).toLocaleDateString('th-TH') : (item.created_at ? new Date(item.created_at).toLocaleDateString('th-TH') : '-')}</td>
        <td>
    ...
    tbody.innerHTML = allItems.map(item => {
      ...
      return `<tr>
        ...
        <td class="text-muted">${item.description || item.category || '-'} | ${item.phone || item.contact_info || '-'}</td>
        <td>
          <div class="action-buttons">
            <button class="action-btn edit" onclick="editItem('${item.id}')"><i class="fa-solid fa-pen"></i></button>
            <button class="action-btn" onclick="deleteItem('${item.id}')" style="color:#e11d48;"><i class="fa-solid fa-trash"></i></button>
...
window.openAddModal = function() {
    ...
    if (isWork) {
        ...
        <div class="form-group row mb-3">
            <div class="col-6"><label>หมวดหมู่สินค้า (เก็บลง Description)</label><input type="text" id="sCat" class="form-control"></div>
...
window.editItem = function(id) {
    ...
    if (isWork) {
        ...
        <div class="form-group row mb-3">
            <div class="col-6"><label>หมวดหมู่สินค้า (เก็บลง Description)</label><input type="text" id="sCat" class="form-control" value="${item.description || item.category || ''}"></div>
            <div class="col-6"><label>เบอร์ติดต่อ</label><input type="text" id="sContact" class="form-control" value="${item.phone || item.contact_info || ''}"></div>
        </div>
        <div class="form-group mb-3"><label>ที่ตั้ง</label><input type="text" id="sLoc" class="form-control" value="${item.location || ''}"></div>
        <div class="form-group mb-3"><label>เว็บไซต์ / แผนที่</label><input type="text" id="sLink" class="form-control" value="${item.website_url || item.link || item.website || ''}"></div>
...
window.saveItemData = async function() {
    ...
    if (isWork) {
        ...
        payload = {
            ...
            description: document.getElementById('sCat').value,
            phone: document.getElementById('sContact').value,
            location: document.getElementById('sLoc').value,
            website_url: document.getElementById('sLink').value,
...
window.deleteItem = async function(id) {
    ...
    try {
        const tableName = currentTab === 'works' ? 'our_work' : 'shops';
        const { error, data } = await supabaseClient.from(tableName).delete().eq('id', id).select();
        if(error) throw error;
        if(!data || data.length === 0) throw new Error("ลบไม่สำเร็จ (โปรดตรวจสอบสิทธิ์ Policy (RLS) ของตารางว่าเปิดให้ลบ DELETE หรือไม่)");
