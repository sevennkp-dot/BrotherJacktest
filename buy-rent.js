...
async function checkAuth() {
  ...
  if (session) {
    ...
    window.currentUserData = customerData;

    const { data: techStatus } = await supabaseClient
      .from("technicians")
      .select("id")
      .eq("id", user.id)
      .single();
      
    const { data: profileResult } = await supabaseClient
      .from("profiles")
      .select("role")
      .eq("id", user.id)
      .single();
    ...
    const userName = customerData?.name || user.email;
...
window.handleAction = function (type, id) {
  ...
  if (window.currentUserData) {
    document.getElementById('fullname').value = window.currentUserData.name || '';
    document.getElementById('phone').value = window.currentUserData.phone || '';
  } else {
