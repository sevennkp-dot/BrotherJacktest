...
async function checkAuthAndLoadData() {
  try {
    ...
    await loadRealCustomers(); supabaseClient.channel('admin-customers-live').on('postgres_changes', { event: '*', schema: 'public', table: 'profiles' }, payload => { loadRealCustomers(); }).subscribe();
  } catch (err) {
    console.error(err);
  ...
  tbody.innerHTML = customers.map(user => {
    ...
    return `
      <tr>
        ...
        <td>
          <div style="font-size:13px"><i class="fa-solid fa-phone text-muted mr-1"></i> ${user.phone || '-'}</div>
          <div style="font-size:13px; margin-top:4px"><i class="fa-regular fa-envelope text-muted mr-1"></i> ${user.email || '-'}</div>
        </td>
        <td>${roleBadge}</td>
...
window.viewCustomer = function(id) {
  ...
  document.getElementById('modalBodyCust').innerHTML = `
    <div class="text-center">
      ...
      <p class="text-muted">${cust.email || '-'} | ${cust.phone || '-'}</p>
    </div>
...
window.saveCustomerData = async function() {
  ...
  try {
     const { error } = await supabaseClient
       .from('profiles')
       .update({ name: newName, phone: newPhone, role: newRole })
