...
const subCategoryData = {
  "ช่างตามบ้าน": ["ไม้", "ไฟฟ้า", "ประปา", "ปูน", "ช่างโครงสร้าง", "ช่างรางน้ำ", "ช่างหลังคา", "ช่างโซล่าเซลล์", "ช่างเชื่อม", "ช่างสี", "ช่างถมดิน", "ช่างแอร์", "ช่างต้นไม้"],
...
window.updateAdminSubCategories = function(existingExpertise = "") {
  const category = document.getElementById('editCategory').value;
  const grid = document.getElementById('adminSubCategoryGrid');
  const wrapper = document.getElementById('adminSubCategoryWrapper');

  grid.innerHTML = '';
  
  if (category && subCategoryData[category]) {
    wrapper.style.display = 'block';
    const expertiseArray = existingExpertise.split(',').map(s => s.trim());
    
    subCategoryData[category].forEach(sub => {
      const isChecked = expertiseArray.includes(sub);
      ...
      checkbox.name = 'adminSubCategory';
      checkbox.value = sub;
      checkbox.id = 'sub-' + sub;
...
window.editTech = function(id) {
  ...
  document.getElementById('modalBody').innerHTML = `
    <div style="max-height: 500px; overflow-y: auto; padding-right: 10px;">
      <div class="mb-4">
        ...
        <div class="form-row" style="display:flex; gap:15px; margin-bottom:15px;">
           ...
           <div style="flex:1;">
             ...
             <select id="editCategory" class="form-control" onchange="updateAdminSubCategories()">
               <option value="ช่างตามบ้าน" ${tech.category === 'ช่างตามบ้าน' ? 'selected' : ''}>ช่างตามบ้าน</option>
               <option value="ช่างยนต์" ${tech.category === 'ช่างยนต์' ? 'selected' : ''}>ช่างยนต์</option>
               <option value="ช่างทำฟาร์มทำโรงเรือน" ${tech.category === 'ช่างทำฟาร์มทำโรงเรือน' ? 'selected' : ''}>ช่างทำฟาร์มทำโรงเรือน</option>
               <option value="แม่บ้านพี่เลี้ยงเด็กและคนชรา" ${tech.category === 'แม่บ้านพี่เลี้ยงเด็กและคนชรา' ? 'selected' : ''}>แม่บ้านพี่เลี้ยงเด็กและคนชรา</option>
               <option value="ช่างอิเล็คทรอนิกส์และ iot" ${tech.category === 'ช่างอิเล็คทรอนิกส์และ iot' ? 'selected' : ''}>ช่างอิเล็คทรอนิกส์และ iot</option>
               <option value="ช่างสารสนเทศ" ${tech.category === 'ช่างสารสนเทศ' ? 'selected' : ''}>ช่างสารสนเทศ</option>
               <option value="ช่างออเกไนท์(อีเว้นท์)" ${tech.category === 'ช่างออเกไนท์(อีเว้นท์)' ? 'selected' : ''}>ช่างออเกไนท์(อีเว้นท์)</option>
             </select>
           </div>
        </div>

        <div id="adminSubCategoryWrapper" style="display:none; margin-bottom: 20px; background: #f8fafc; padding: 12px; border-radius: 6px; border: 1px solid #e2e8f0;">
           <label class="small font-weight-bold mb-2 d-block">ทักษะความเชี่ยวชาญ (Expertise)</label>
           <div id="adminSubCategoryGrid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;"></div>
        </div>
      ...
      <div class="mb-4">
        ...
        <div class="form-row" style="display:flex; gap:15px; margin-bottom:15px;">
           <div style="flex:1;">
             <label class="small font-weight-bold">ราคาเริ่มต้น (ตัวหนังสือ/ช่วงราคา)</label>
             <input type="text" id="editStartingPrice" class="form-control" value="${tech.starting_price || ''}" placeholder="เช่น 500-700">
           </div>
           <div style="flex:1;">
             <label class="small font-weight-bold">ค่าเดินทาง</label>
             <input type="text" id="editTravelFee" class="form-control" value="${tech.travel_fee || ''}" placeholder="กม. ละ 5 บาท">
           </div>
        </div>
        <div class="form-row" style="display:flex; gap:15px; margin-bottom:15px;">
           <div style="flex:1;">
             <label class="small font-weight-bold">ระยะทางฟรี (กม.)</label>
             <input type="number" id="editFreeDistance" class="form-control" value="${tech.free_distance || 0}">
           </div>
           <div style="flex:1;">
      ...
      <div class="mb-2">
        ...
        <div class="form-group mb-3">
           <label class="small font-weight-bold">จุดเด่นเพิ่มเติม/รายละเอียด (Selling Points)</label>
           <textarea id="editSellingPoints" class="form-control" rows="2">${tech.selling_points || ''}</textarea>
        </div>
        <div class="form-group mb-3">
  ...
  updateAdminSubCategories(tech.expertise || "");
...
window.saveTechData = async function() {
  ...
  const updates = {
    ...
    starting_price: document.getElementById('editStartingPrice').value,
    travel_fee: document.getElementById('editTravelFee').value,
    free_distance: parseInt(document.getElementById('editFreeDistance').value) || 0,
    vehicle: document.getElementById('editVehicle').value,
    expertise: Array.from(document.querySelectorAll('input[name="adminSubCategory"]:checked')).map(cb => cb.value).join(', '),
    selling_points: document.getElementById('editSellingPoints').value,
